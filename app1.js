...
const vehicleColors = [
  0x79c7d4, 0xf0947d, 0xedf39b /*0xa52523, 0xbdb638, 0x78b14b*/,
];

const lawnGreen = "#b1e18e";
const trackColor = "#dbd2d9";
const edgeColor = "#473944";
const treeCrownColor = 0x46993d;
const treeTrunkColor = 0x4d2e2e;
...
renderMap(cameraWidth, cameraHeight * 2.5);
...
if (config.grid) {
  ...
  //const gridMaterial = new THREE.LineBasicMaterial({ color: 0x432d3c }); // Change the color here
...
function reset() {
  ...
  playerAngleMoved = 0;
  score = 0;
  //scoreElement.innerText = "Press the Upward Button to start!";
...
function renderMap(mapWidth, mapHeight) {
  ...
  if (config.trees) {
    const tree1 = Tree();
    tree1.position.x = arcCenterX * 1.4;
    tree1.position.y = arcCenterX * 0.7;
    scene.add(tree1);

    const tree2 = Tree();
    tree2.position.y = arcCenterX * 0.4;
    tree2.position.x = arcCenterX * 1.7;
    scene.add(tree2);

    const tree3 = Tree();
    tree3.position.x = arcCenterX * -2.6;
    tree3.position.y = arcCenterX * 1.3;
    scene.add(tree3);

    const tree4 = Tree();
    tree4.position.x = arcCenterX * -2.2;
    tree4.position.y = arcCenterX * 1.5;
    scene.add(tree4);

    const tree5 = Tree();
    tree5.position.x = -arcCenterX * 2;
    tree5.position.y = arcCenterX * 1.7;
    scene.add(tree5);

    const tree6 = Tree();
    tree6.position.x = -arcCenterX * 2.0;
    tree6.position.y = -arcCenterX * 1.6;
    scene.add(tree6);

    const tree7 = Tree();
    tree7.position.x = -arcCenterX * 2.4;
    tree7.position.y = -arcCenterX * 1.5;
    scene.add(tree7);
  }
}
...
function hitDetection() {
  ...
  const hit = otherVehicles.some((vehicle) => {
    if (vehicle.type == "car") {
      ...
      if (getDistance(playerHitZone1, vehicleHitZone1) < 40) {
        endDOM.style.visibility = "visible";
        return true;
      }
      if (getDistance(playerHitZone1, vehicleHitZone2) < 40) {
        endDOM.style.visibility = "visible";
        return true;
      }

      // Another vehicle hits the player
      if (getDistance(playerHitZone2, vehicleHitZone1) < 40) {
        endDOM.style.visibility = "visible";
        return true;
      }
    }
    ...
    if (vehicle.type == "truck") {
      ...
      if (getDistance(playerHitZone1, vehicleHitZone3) < 40) {
        endDOM.style.visibility = "visible";
        return true;
      }
