...
const vechicleColors = [0x79c7d4, 0xf0947d, 0xedf39b];
...
function Car() {
  ...
  const cabin = new THREE.Mesh(
    ...
    [
      ...
      new THREE.MeshPhongMaterial({ color: 0xaddcda, flatShading: true }),
      new THREE.MeshPhongMaterial({ color: 0xaddcda, flatShading: true }),
    ]
  );
...
function Truck() {
  ...
  const base = new THREE.Mesh(
    ...
    new THREE.MeshLambertMaterial({ color: 0x976873, flatShading: true })
  );
  ...
  const cargo = new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({ color: 0x976873, flatShading: true })
  );
...
function Three() {
  ...
  const trunkWidth = 10 * zoom;
  const trunkHeight = 40 * zoom;
  const crownRadius = 25 * zoom;

  const trunk = new THREE.Mesh(
    new THREE.BoxBufferGeometry(trunkWidth, trunkWidth, trunkHeight),
    new THREE.MeshPhongMaterial({ color: 0x6b5c41, flatShading: true })
  );
  trunk.position.z = trunkHeight / 2; // Position the trunk at half its height
  trunk.castShadow = true;
  trunk.receiveShadow = true;
  three.add(trunk);

  // Create the crown as a circular mesh
  const crown = new THREE.Mesh(
    new THREE.SphereBufferGeometry(crownRadius, 50, 50), // Adjust the radius and segmentation as needed
    new THREE.MeshLambertMaterial({ color: 0x39955c, flatShading: true })
  );

  // Position the crown directly above the trunk
  crown.position.z = trunkHeight + crownRadius; // Place crown above the trunk
...
function Player() {
  ...
  const playerSize = 1; // You need to define the playerSize and zoom variables
  const zoom = 1; // Define your zoom factor here

  // Create the body as a spherical mesh with increased height
  const body = new THREE.Mesh(
    new THREE.SphereBufferGeometry(22 * zoom, 32, 32), // Adjust the radius and segmentation as needed
    new THREE.MeshPhongMaterial({ color: 0xfcf6fe, flatShading: true })
  );
  body.position.z = 10 * zoom;
  player.add(body);

  // Create the rowel as a spherical mesh
  const rowel = new THREE.Mesh(
    new THREE.SphereBufferGeometry(13 * zoom, 20, 20), // Adjust the radius and segmentation as needed
    new THREE.MeshLambertMaterial({ color: 0x8c6c96, flatShading: true })
  );
  ...
  // Assuming you have a scene, camera, and light setup
  // Add the player to the scene here if not already done
...
function Road() {
  ...
  const middle = createSection(0xbcb5c0);
  middle.receiveShadow = true;
  road.add(middle);

  const left = createSection(0x938b97);
  left.position.x = -boardWidth * zoom;
  road.add(left);

  const right = createSection(0x938b97);
...
function Grass() {
  ...
  const middle = createSection(0x9cf68a);
  middle.receiveShadow = true;
  grass.add(middle);

  const left = createSection(0x82ca74);
  left.position.x = -boardWidth * zoom;
  grass.add(left);

  const right = createSection(0x82ca74);
  right.position.x = boardWidth * zoom;
  grass.add(right);
